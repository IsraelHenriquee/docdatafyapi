// Playground (Testar): lembra phone_number_id e waba_id entre aberturas e páginas,
...
(() => {
  ...
  // O placeholder do Mintlify muda com o idioma ("enter phone_number_id",
  // "digite phone_number_id"), mas termina sempre com o nome do campo.
  const campoDoInput = (el) => {
    if (!(el instanceof HTMLInputElement)) return null
    const ultimaPalavra = (el.getAttribute('placeholder') || '').trim().split(/\s+/).pop()
    return CAMPOS.includes(ultimaPalavra) ? ultimaPalavra : null
  }
  ...
  const varrer = (raiz) => raiz.querySelectorAll?.('input[placeholder]').forEach(restaurar)
